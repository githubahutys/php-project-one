...
class EasyMySQLi extends mysqli {
    ...
    public function prepareArray($array) {
        ...
        foreach ($array as $k => $v) {
            if (is_numeric($v))	$new[$k] = str2num($v, false);
            else $new[$k] = $v;
        }
        return $new;
    }
    ...
    private function _bindRes ($type=false, $limit=false) {
        ...
        if (($type & EMYSQLI_BIND_ASSOC) == EMYSQLI_BIND_ASSOC) {
            $fields = $this->_stmt->result_metadata()->fetch_fields();
            if (!$fields) if ($this->exceptions) throw new MySQLiException('Could not fetch field metadata');
            else return null;
            $cnt = count($fields);
            if (!is_array($fields)) {
...
$mysql = new EasyMySQLi('localhost', 'root', '', 'hotelms');
